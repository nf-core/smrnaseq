...
data_dir=${data_path}/nf-core/smrnaseq_test_set
if [ -d $data_dir ]
then
    echo "Found existing test set, using $data_dir"
else
    echo "Downloading test set..."
    curl https://export.uppmax.uu.se/b2013064/test-data/nf-core/smrnaseq_test_set.tar.bz2 > ${data_path}/nf-core/smrnaseq_test_set.tar.bz2
    echo "Unpacking test set..."
    tar xvjf ${data_path}/nf-core/smrnaseq_test_set.tar.bz2 -C ${data_path}
    echo "Done"
fi

nf_cmd="nextflow run $script_path -resume -profile docker --mature ${data_dir}/mature.fa --hairpin ${data_dir}/hairpin.fa --bt2index ${data_dir}/genome --gtf ${data_dir}/genes.gtf --reads \"${data_dir}/*.R1.fastq.gz\" --saveReference"
